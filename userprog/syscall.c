...
static bool is_vaddr_valid(void *vaddr);
...
static void remove_hander(struct intr_frame *f);
...
void syscall_init(void) {
  ...
  memset(syscall_handlers, 0,
         sizeof(syscall_handlers)); // sycall 함수배열 초기화
...
static bool is_code_segment(void *addr, unsigned size) {
  struct thread *t = thread_current();
  struct page *page;

  for (void *p = addr; p < addr + size; p += PGSIZE) {
    page = spt_find_page(&t->spt, p);
    if (page != NULL && page->writable == false) {
      return true; // 읽기 전용 페이지를 발견하면 코드 세그먼트로 간주
    }
  }
...
static void read_handler(struct intr_frame *f) {
  ...
  if (is_code_segment(buffer, size)) {
    thread_exit_by_error(-1);
  }
...
static struct file *get_file_by_fd(int fd) {
  ...
  if (is_empty(fd_table, fd))
    return NULL;
  // todo 유효한 파일인지 검사, 이미 닫혔는지 아닌지, 가르키는 주소가 파일이
  // 맞는지.
